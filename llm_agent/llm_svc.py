"""
framework/llm_agent/llm_svc.py  (llm_image copy)

Single entry-point for calling any supported LLM CLI tool.
This copy lives inside llm_image/ and is used by the llm-svc container.
It does not include the LLM_SVC_URL remote-fallthrough logic — the
container itself IS the remote service.
...
Raises:
    ...
    FileNotFoundError — CLI binary not on PATH
...
def run_once(
    ...
    *,
    ...
    Args:
        ...
        timeout:  Seconds before the process is killed (default 1800).
